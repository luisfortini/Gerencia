...
export const InstanciasPage = () => {
  ...
  const { data: instanciasData = [], isLoading, isFetching, refetch } = useInstancias();
  ...
  const [metaPrimeiraResposta, setMetaPrimeiraResposta] = useState("");

  const [metaFeedback, setMetaFeedback] = useState<FeedbackEntry | null>(null);
  ...
  const { isSuperAdmin, isAdmin } = useMemo(() => {
    ...
    try {
      ...
      console.warn("Não foi possível interpretar os dados do usuário salvos localmente.", error);
  ...
  useEffect(() => {
    ...
    if (dashboardSettings.data) {

      setMetaPrimeiraResposta(String(dashboardSettings.data.metaPrimeiraRespostaMin));

    }

  }, [dashboardSettings.data?.metaPrimeiraRespostaMin]);



  const isSavingDashboardMeta = updateDashboardSettings.isPending;

  const metaDefaultValue = dashboardSettings.data?.defaultMetaPrimeiraRespostaMin ?? 25;

  const metaParsedValue = Number.parseInt(metaPrimeiraResposta, 10);

  const isMetaReady = Boolean(dashboardSettings.data);

  const canEditMeta = canManageInstancias;



  const handleMetaSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const normalized = metaPrimeiraResposta.trim();
    ...
    if (!Number.isFinite(parsed) || parsed < 1) {
      ...
      setMetaFeedback({
        ...
        message: "Informe um numero inteiro de minutos maior ou igual a 1.",
    ...
    const clamped = Math.min(1440, parsed);



    try {

      const updated = await updateDashboardSettings.mutateAsync({

        metaPrimeiraRespostaMin: clamped,

      });



      setMetaPrimeiraResposta(String(updated.metaPrimeiraRespostaMin));

      setMetaFeedback({ type: "success", message: "Meta de primeira resposta atualizada." });

    } catch (error) {

      const message = resolveErrorMessage(error, "Não foi possível atualizar a meta.");

      setMetaFeedback({ type: "error", message });
  ...
  const handleResetMeta = async () => {
    ...
    if (!canEditMeta || !dashboardSettings.data) {
    ...
    const fallback = dashboardSettings.data.defaultMetaPrimeiraRespostaMin;

    setMetaPrimeiraResposta(String(fallback));
    ...
    try {
      ...
      await updateDashboardSettings.mutateAsync({ metaPrimeiraRespostaMin: fallback });

      setMetaFeedback({ type: "success", message: "Meta restaurada para o padrao." });

    } catch (error) {

      const message = resolveErrorMessage(error, "Não foi possível restaurar a meta.");
  ...
  const openModal = () => {
    ...
    if (!canManageInstancias) {
      ...
      window.alert("Somente administradores podem cadastrar instâncias.");
  ...
  const fetchAndApplyQr = async (
    ...
    if (!canManageInstancias) {
      ...
      window.alert("Somente administradores podem executar esta ação.");
    ...
    try {
      ...
      if (qr) {
        ...
        const message = "Evolution não retornou um QRCode.";
      ...
      const message = resolveErrorMessage(error, "Não foi possível gerar o QR Code.");
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const message = resolveErrorMessage(error, "Não foi possível criar a instância.");
  ...
  const handleSyncWebhook = async (instancia: InstanciaWhatsapp) => {
    ...
    try {
      ...
      const message = resolveErrorMessage(error, "Não foi possível sincronizar o webhook.");
  ...
  const handleRefreshStatus = async (instancia: InstanciaWhatsapp) => {
    ...
    try {
      ...
      const message = resolveErrorMessage(error, "Não foi possível atualizar o status.");
  ...
  const handleDeleteInstancia = async (instancia: InstanciaWhatsapp) => {
    ...
    const confirmed = window.confirm(
      ...
      `Excluir a instância "${instancia.iwh_nome}"? Essa ação remove o vínculo local e não pode ser desfeita.`,
    );
    ...
    try {
      ...
      const message = resolveErrorMessage(error, "Não foi possível excluir a instância.");
  ...
  const renderModalContent = () => {
      ...
      return (
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-2 text-sm text-subtle">
            ...
            <span>Gerando QRCode e aguardando a conexão com o Evolution.</span>
          ...
          <p className="text-xs text-muted-foreground">
            ...
            Assim que a conexão for concluida, esta janela sera atualizada automaticamente.
    ...
    return (
      ...
      <div className="space-y-3 text-sm text-subtle">
        ...
        <p className="font-medium text-red-600">Não foi possível concluir a conexão.</p>
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardHeader className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              title={!canManageInstancias ? "Somente administradores podem cadastrar instâncias." : undefined}
        ...
        <CardContent className="space-y-2 text-sm text-subtle">
          ...
          <p>
            ...
            Cadastre uma instância para gerar o QRCode e conectar o WhatsApp. Após a conexão, voce pode
          ...
          <p>
            ...
            {isLimitReached
              ...
              ? "Limite atingido para novas instâncias."
      ...
      <Card>
        ...
        <CardHeader>
          ...
          <CardTitle>Meta de primeira resposta</CardTitle>

          <CardDescription>Defina o SLA utilizado nos indicadores do dashboard.</CardDescription>
          ...
          <form onSubmit={handleMetaSubmit} className="space-y-4">
            ...
            <div className="flex flex-col gap-3 md:flex-row md:items-end">
              ...
              <div className="flex-1 space-y-1">
                ...
                <label className="text-xs font-medium text-subtle" htmlFor="meta-primeira-resposta">

                  Meta (minutos) - Valor padrao: {metaDefaultValue} minutos.

                </label>

                <Input

                  id="meta-primeira-resposta"
                  ...
                  value={metaPrimeiraResposta}

                  onChange={(event) => setMetaPrimeiraResposta(event.target.value)}

                  disabled={!isMetaReady || dashboardSettings.isLoading || !canEditMeta}

                />

                {/* <p className="text-xs text-muted-foreground">Valor padrao: {metaDefaultValue} minutos.</p> */}
              ...
              <div className="flex gap-2 md:self-end">
                ...
                <Button type="submit" disabled={!canEditMeta || isSavingDashboardMeta || !isMetaReady}>

                  {isSavingDashboardMeta ? "Salvando..." : "Salvar meta"}
                ...
                <Button
                  ...
                  disabled={
                    ...
                    Number.isNaN(metaParsedValue) ||

                    metaParsedValue === metaDefaultValue
            ...
            {metaFeedback ? (
              ...
              <p className={`text-xs ${metaFeedback.type === "error" ? "text-red-600" : "text-green-600"}`}>
            ...
            {dashboardSettings.isLoading && !isMetaReady ? (

              <p className="text-xs text-muted-foreground">Carregando meta configurada...</p>
                  ...
                  <div key={instancia.iwh_id} className="rounded-lg border border-border bg-surface p-4 shadow-sm">
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      ...
                      <Button
                        ...
                        title={!canManageInstancias ? "Somente administradores podem executar esta ação." : undefined}
      ...
      <Modal
        ...
        title={{
          ...
          form: "Nova instância Evolution",

          connecting: modalInstance ? `Conectando ${modalInstance.iwh_nome}` : "Conectando instância",
        ...
        description={
          ...
          modalStage === "form"
            ...
            ? "Informe os dados da instância e avance para gerar o QRCode de conexão."
